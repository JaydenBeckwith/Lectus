...
export default function AddView({ pdfLoading, pdfStatus, onPdfUpload, onDoiLookup, hasKey, onConnect, theme }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "1.5rem", maxWidth: 580, margin: "0 auto", width: "100%" }}>
      <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: "1.4rem", fontStyle: "italic", color: theme.textBright, marginBottom: "0.3rem" }}>Add a paper</div>
      ...
      {!hasKey && (
        <div style={{ marginBottom: "1rem", marginLeft: "-0.25rem", marginRight: "-0.25rem" }}>
          <NoKeyBanner onConnect={onConnect} theme={theme} />
        </div>
      )}

      <form onSubmit={handleDoiSubmit} style={{ background: theme.panel, border: `1px solid ${theme.panelBorder}`, borderRadius: 12, padding: "1rem 1.1rem", marginBottom: "1rem" }}>
        <div style={{ fontSize: "0.65rem", color: theme.textMuted, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "0.5rem" }}>Quick add by DOI</div>
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <input value={doi} onChange={(e) => setDoi(e.target.value)} placeholder="10.1038/s41591-018-0274-4" disabled={doiLoading}
            style={{ flex: 1, background: theme.inputBg, border: `1px solid ${theme.inputBorder}`, borderRadius: 8, padding: "0.5rem 0.75rem", color: theme.text, fontSize: "0.82rem", fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace" }} />
          <button type="submit" disabled={doiLoading || !doi.trim()} style={{
            background: doi.trim() && !doiLoading ? theme.accent : theme.chip,
            color: doi.trim() && !doiLoading ? theme.bg : theme.textMuted,
            border: "none", borderRadius: 8, padding: "0.5rem 1rem", fontSize: "0.78rem",
            cursor: doi.trim() && !doiLoading ? "pointer" : "not-allowed", fontWeight: 500,
          }}>{doiLoading ? "…" : "Fetch"}</button>
        </div>
        {doiStatus && (
      ...
      <div onClick={() => !pdfLoading && fileInputRef.current?.click()}
        onMouseEnter={(e) => { if (!pdfLoading) { e.currentTarget.style.background = theme.panelHover; e.currentTarget.style.borderColor = theme.accent; } }}
        onMouseLeave={(e) => { e.currentTarget.style.background = theme.panel; e.currentTarget.style.borderColor = theme.panelBorder; }}
        style={{ background: theme.panel, border: `2px dashed ${theme.panelBorder}`, borderRadius: 12, padding: "2.5rem 1.5rem", textAlign: "center", cursor: pdfLoading ? "not-allowed" : "pointer", transition: "all .2s", opacity: pdfLoading ? 0.5 : 1 }}>
